...
def make_commkey(key, session_id, ticks=50):
    ...
    k = unpack(b"BBBB", k)
    k = pack(
        b"BBBB", k[0] ^ ord("Z"), k[1] ^ ord("K"), k[2] ^ ord("S"), k[3] ^ ord("O")
    )
    k = unpack(b"HH", k)
    k = pack(b"HH", k[1], k[0])
...
class ZK(object):
    ...
    def __create_tcp_top(self, packet):
        ...
        length = len(packet)
        top = pack(
            "<HHI", const.MACHINE_PREPARE_DATA_1, const.MACHINE_PREPARE_DATA_2, length
        )
        return top + packet
    ...
    def __test_tcp_top(self, packet):
        ...
        if len(packet) <= 8:
            if self.verbose:
                print("incomplete response? " + codecs.encode(packet, "hex"))
            return 0
        tcp_header = unpack("<HHI", packet[:8])
        if (
            tcp_header[0] == const.MACHINE_PREPARE_DATA_1
            and tcp_header[1] == const.MACHINE_PREPARE_DATA_2
        ):
            return tcp_header[2]
        return 0
    ...
    def __str__(self):
        ...
        """
        return (
            "ZK %s://%s:%s users[%i]:%i/%i fingers:%i/%i, records:%i/%i faces:%i/%i"
            % (
    ...
    def write_lcd(self, line_number, text):
        ...
        command_string = (
            pack(b"<hb", line_number, 0)
            + b" "
            + text.encode(self.encoding, errors="ignore")
        )
        cmd_response = self.__send_command(command, command_string)
        if cmd_response.get("status"):
            return True
    ...
    def set_user(
        ...
        if self.user_packet_size == 28:  # self.firmware == 6:
            ...
            name_pad = name.encode(self.encoding, errors="ignore").ljust(24, b"\x00")[
                :24
            ]
    ...
    def get_templates(self):
        ...
        if self.fingers == 0:
            return []
        templates = []
        templatedata, size = self.read_with_buffer(
            const.CMD_DB_RRQ, const.FCT_FINGERTMP
        )
        if size < 4:
            if self.verbose:
        ...
        if self.verbose:
            print(
                "get template total size {}, size {} len {}".format(
                    total_size, size, len(templatedata)
                )
            )
        ...
        if self.user_packet_size == 28:
            while len(userdata) >= 28:
                (
                    ...
                    timezone,
                    user_id,
                ) = unpack("<HB5s8sIxBhI", userdata.ljust(28, b"\x00")[:28])
                if uid > max_uid:
                    max_uid = uid
            ...
            try:
                ...
                while len(data) >= 12:
                    if len(data) == 12:
                        ...
                        user_id, status, punch, timehex, _other = unpack(
                            "<24sBB6s4s", data[:36]
                        )
                        data = data[36:]
                    elif len(data) >= 52:
    ...
    def __recieve_tcp_data(self, data_recv, size):
        ...
        if (tcp_length - 8) < size:
            ...
            if self.verbose:
                print(
                    "for misssing {} recieved {} with extra {}".format(
    ...
    def __recieve_chunk(self):
        ...
        if self.__response == const.CMD_DATA:
            if self.tcp:
                if self.verbose:
                    print(
                        "_rc_DATA! is {} bytes, tcp length is {}".format(
    ...
    def read_with_buffer(self, command, fct=0, ext=0):
        ...
        if cmd_response["code"] == const.CMD_DATA:
            if self.tcp:
                if self.verbose:
                    print(
                        "DATA! is {} bytes, tcp length is {}".format(
                            len(self.__data), self.__tcp_length
                        )
                    )
                if len(self.__data) < (self.__tcp_length - 8):
                    ...
                    if self.verbose:
                        print("need more data: {}".format(need))
                    more_data = self.__recieve_raw_data(need)
                    return b"".join([self.__data, more_data]), len(self.__data) + len(
                        more_data
                    )
                else:
                    if self.verbose:
        ...
        if self.verbose:
            print(
                "rwb: #{} packets of max {} bytes, and extra {} bytes remain".format(
                    packets, MAX_CHUNK, remain
                )
            )
        for _wlk in range(packets):
